...
description: "Workflow 1.5: Bridge between idea discovery and auto review. Reads an experiment plan, implements experiment code, deploys to GPU, and collects initial results. Use this whenever the user wants to move from plan to execution: implement experiments, wire up code, start running, deploy the plan, launch jobs, or turn a proposal into runnable experiments. Trigger on phrases like \"实现实验\", \"从计划到跑实验\", \"开始做实验\", \"把这个方案跑起来\", \"deploy the plan\", \"implement experiments\", \"bridge this plan\", even if the user only says they have a plan and now need to run it."
...
This skill must choose its external reviewer backend from the tools available at runtime:

- If `mcp__claude-review__review_start` is available, use the `claude-review` MCP bridge as the reviewer backend. Target reviewer: `claude-opus-4-6` with `max` effort.
- Otherwise, if `mcp__codex__codex` is available, use Codex MCP as the reviewer backend with `gpt-5.4` plus `config: {"model_reasoning_effort": "xhigh"}`.
- Never silently collapse to same-family self-review when the intended external bridge is missing.
- This runtime-selection rule overrides any later single-backend examples in the file.
