...
| 阶段性任务完成 / 新 session 交接 | `PROJECT_CONTEXT.md` 的 Session Handoff · 必要时 `~/.claude/memory/active-tasks.json` |
| 研究口径 / 项目战略变化 | `PROJECT_CONTEXT.md` 当前定位 · `CLAUDE.md` / `AGENTS.md` 长期约束 · 对外 docs 中相关章节 |
...
| 推翻的决策 | 删除旧条目,留新决策 |
| 跨会话只用一次的临时上下文 | 删除 |
| 用户明确纠正或三次同根失败 | 写入 `/Users/yangjingchi/.claude/memory/patterns.md` |
